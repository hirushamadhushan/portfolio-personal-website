...
Contact details, education and live-demo links are all filled in from `RDHM_Dharmakeerthi_CV_7.pdf`.
...
| `assets/RDHM_Dharmakeerthi_CV_7.pdf` | Download CV button | The `download="Hirusha-Madhushan-CV.pdf"` attribute on the button renames it on the visitor's machine, so the file here can keep any name. Swap the filename in both places if you replace it. |
